...
int main(int argc, char **argv) {
	...
	EtherDreamTester tester = EtherDreamTester();

	i = 0;
	while (1) {
		tester.fill_circle((float)i / 50, mode);
		int res = etherdream_write(d, tester.circle, CIRCLE_POINTS, 30000, 1);
		if (res != 0) {
			printf("write %d\n", res);
		}
